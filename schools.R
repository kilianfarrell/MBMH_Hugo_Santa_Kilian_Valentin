...
post_theta = function(nchain, prop_sd){
  ...
  for (i in 2:nchain){
    ...
    top = -0.5*(-(prop*S)+(prop*sqrt(0.0001))^2 + (exp(prop)*sum(exp(phi*LRT)%*%t(Y - mu)%*%(Y - mu))))
    bottom = -0.5*(-(current*S)+(current*sqrt(0.0001))^2 + (exp(current)*sum(exp(phi*LRT)%*%t(Y - mu)%*%(Y - mu))))
...
post_phi = function(nchain, init, prop_sd){
  ...
  for (i in 2:nchain){
    ...
    top = -0.5*(-(prop*sum(LRT))+(prop*sqrt(0.0001))^2 + (exp(theta)*sum(exp(prop*LRT)%*%t(Y - mu)%*%(Y - mu))))
    bottom = -0.5*(-(current*sum(LRT))+(current*sqrt(0.0001))^2 + (exp(theta)*sum(exp(current*LRT)%*%t(Y - mu)%*%(Y - mu))))
